...
derives fact \code{visited(1)} and a \code{visit(b)} for all edges $(1, b)
...
infer several \code{visit} facts. The second rule of the program
...
the old \code{pagerank} fact and then derives an aggregate. The first
argument contains \code{sum} (the aggregate operator) and $V$ (the
aggregate value). The second argument \code{edge(A, B), pagerank(B, V)}
is the body of the aggregate that is iterated over to derive \code{edge(A, B),
pagerank(B, V)}. The values $V_i$ of each combination of \code{edge}
and \code{pagerank} are summed up and instantiated as $V = \sum_i V_i$ in
\code{pagerank(A, damping/PAGES + (1.0 - damping) * V)} in order to compute
...
Comprehensions and aggregates only appear on the head of the rule and can only
use simple facts.
